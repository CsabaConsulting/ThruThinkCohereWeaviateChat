...
That's more than nothing, however users can ask so wide variety of questions that chunking these documents may not provide a nice ground for a good vector match in the embedding space. To increase the performance of the chat agent I employ several techniques.
...
4. We'll take top k of those documents and perform final two RAG calls which supply the displayed data.
...
3. I added capability for my script to produce fine tuning file for the Cohere LLM model (see https://github.com/CsabaConsulting/question_extractor/blob/main/fine_tune_prep.py).
...
## Future plans:
1. Decrease runtime by running the variation documetn retrievals in parallel, this is Streamlit asyncio aawait challenge.
2. Decrease runtime by running the final two co.chat RAG calls in parallel, this is Streamlit asyncio aawait challenge.
3. Make the citation linking nicer.
4. Measure how much the RAG Fusion (if at all) improves answer quality. Measure the trade-off factroing in extra latency and potential token usage increase which also means cost increase.
5. Integrate the agent into ThruThink which uses ASP.NET MVC / C# / Azure technology stack, but not open source. In that final deployment I'll be able to open up referred help topics using the meta-data I get back as part of the query results document meta data.
